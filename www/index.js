...
import { vsSource } from "./js/shaders/vertex";
import { fsSource } from "./js/shaders/fragment";
import { hexToGlColor, loadShader } from "./js/shaders/utils";
import { COLOR, POSITION, RESOLUTION, SIZE } from "./js/shaders/consts";
...
// Si la création du shader échoue on alerte
if (!gl.getProgramParameter(shaderProgram, gl.LINK_STATUS)) {
    alert(`Impossible d'initialiser le shader : ${gl.getProgramInfoLog(shaderProgram)}`);
    //return null;
}
...
const sizeUniformLocation = gl.getUniformLocation(shaderProgram, SIZE);
gl.uniform1f(resolutionUniformLocation, CELL_SIZE);

const colorUniformLocation = gl.getUniformLocation(shaderProgram, COLOR);

// dessin des cellules
gl.bindBuffer(gl.ARRAY_BUFFER, cellVertexBuffer);
gl.vertexAttribPointer(positionAttribLocation, 2, gl.FLOAT, false, 0, 0);
gl.uniform4f(colorUniformLocation, ...hexToGlColor(DEAD_COLOR));
gl.drawArrays(gl.POINTS, 0, cellVertices.length / 2);

// dessin de la grille
gl.bindBuffer(gl.ARRAY_BUFFER, gridVertexBuffer);
gl.vertexAttribPointer(positionAttribLocation, 2, gl.FLOAT, false, 0, 0);
gl.uniform4f(colorUniformLocation, ...hexToGlColor(GRID_COLOR));
